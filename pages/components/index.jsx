...
function Components({elements, sections, components}) {
    return (
        <div className="">
            <div style={{minHeight: "90vh", display: "grid", alignItems: "start", alignContent: "start", marginTop: "",}}className="x-container">
            <div className="areaGrid"> <div> <h3  className="page-title" style={{color: "white", marginTop: "rem"}}><PuzzlePiece />Components</h3>
                ...
                {components.map((component, index) =>
                  <code key={index} style={{display: "grid", textAlign: "left", marginBottom: "2rem", justifyItems: "start"}}>
                     ...
                     <ElementItem itemType="title" item={component.title} />
                     <ElementItem itemType="name" item={component.name.current} />
                     <ElementType />
                     <ElementFields itemType="fields" fields={component.field} />
  ...
  <div className="queryZone">  <p className="whatQuery">*[_type == "xComponentGenerator"]</p>
  <JSONPretty style={{ overflowX: "scroll"}} id="json-pretty" data={components}></JSONPretty>
  </div>
  </div>
